...
describe("application routing", () => {
  ...
  it("builds matching history routes", () => {
    expect(pathHref("/learn/en/short-stories")).toBe("/learn/en/short-stories");
    expect(viewPath({ page: "english" })).toBe("/learn/en");
  ...
  it("leaves a canonical lesson before navigating back to a section", () => {
    ...
    navigateToPath("/learn/en/short-stories/section-1");
    expect(window.location.pathname).toBe("/learn/en/short-stories/section-1");
  ...
  it("migrates legacy hash deep links without adding a history entry", () => {
    ...
    expect(window.location.pathname).toBe("/learn/en/short-stories");
    expect(window.location.hash).toBe("");
  });
});
