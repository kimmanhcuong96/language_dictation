...
export function LessonTranslationPanel({lessonId,sentenceId,locale}:{lessonId:string;sentenceId:string;locale:UiLocale}){
  ...
  return <section className="lesson-translation-panel">
    <header><div><Languages size={18}/><h2>{t("translation")}</h2></div><select aria-label={t("targetLanguage")} value={target} onChange={event=>selectLanguage(event.target.value)}>{languages.map(item=><option key={item.code} value={item.code}>{item.nativeName} · {item.code}</option>)}</select></header>
