...
interface Props { src: string; startMs: number; endMs: number; locale: UiLocale; playbackRate?: number; repeat?: boolean; repeatDelayMs?: number; paddingMs?: number; onError?: () => void; onPlaybackComplete?: () => void; }
...
export const AudioSegmentPlayer = forwardRef<AudioSegmentPlayerHandle, Props>(function AudioSegmentPlayer({ src, startMs, endMs, locale, playbackRate = 1, repeat = false, repeatDelayMs = 0, paddingMs = 50, onError, onPlaybackComplete }, ref) {
  const audioRef = useRef<HTMLAudioElement>(null); const frameRef = useRef<number | undefined>(undefined); const repeatTimeoutRef = useRef<number | undefined>(undefined); const sessionRef = useRef(createPlaybackSessionController()); const completionRef = useRef(onPlaybackComplete);
  completionRef.current = onPlaybackComplete;
  ...
  const invalidate = () => { sessionRef.current.invalidate(); stopLoop(); };
  const pause = () => { invalidate(); audioRef.current?.pause(); setPlaying(false); };
  const monitor = (sessionId: number) => { if (!sessionRef.current.isActive(sessionId)) return; const audio = audioRef.current; if (!audio) return; const virtual = toVirtualTime(audio.currentTime, bounds.startSeconds, bounds.durationSeconds); setCurrent(virtual); if (audio.currentTime >= bounds.endSeconds - .01) { audio.pause(); setPlaying(false); stopLoop(); completionRef.current?.(); if (repeat) repeatTimeoutRef.current=window.setTimeout(() => { if (sessionRef.current.isActive(sessionId)) replay(); },repeatDelayMs); else { audio.currentTime = bounds.endSeconds; setCurrent(bounds.durationSeconds); } return; } frameRef.current = requestAnimationFrame(() => monitor(sessionId)); };
  const replay = () => { const audio = audioRef.current; if (!audio || error) return; invalidate(); const sessionId=sessionRef.current.begin(); audio.currentTime = bounds.startSeconds; audio.playbackRate = playbackRate; void audio.play().then(() => { if (!sessionRef.current.isActive(sessionId)) { audio.pause(); return; } setPlaying(true); frameRef.current = requestAnimationFrame(() => monitor(sessionId)); }).catch(() => { if (!sessionRef.current.isActive(sessionId)) return; invalidate(); setPlaying(false); setError(true); onError?.(); }); };
  const toggle = () => playing ? pause() : replay();
  useImperativeHandle(ref, () => ({ replay, toggle }));
  useEffect(() => { const audio = audioRef.current; pause(); setCurrent(0); setError(false); if (audio) audio.currentTime = bounds.startSeconds; return () => { invalidate(); audio?.pause(); }; }, [src, bounds.startSeconds, bounds.endSeconds]);
