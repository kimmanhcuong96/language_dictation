  ...
  return <AdminShell onHome={onHome} locale={locale}>
    ...
    {mode==="package"&&<section>
      {batch ? <BatchPreview locale={locale} batch={batch} busy={busy} onConfirm={() => void runBatch(false)} onRetry={() => void runBatch(true)} onReset={resetBatch} /> : <form className="admin-import" onSubmit={event => { event.preventDefault(); void validateBatch(); }}>
        ...
        <p className="selected-section">{youtubeImportT(locale,"properNamesConvention")}</p>
        ...
        {inputMethod === "files" ? <label>{t("lessonResources")}<input required type="file" multiple accept=".mp3,.srt,.txt,.json,audio/mpeg,application/x-subrip,text/plain,application/json" onChange={event => setFiles(Array.from(event.target.files ?? []))} /><small>{t("directFilesHint")}</small></label> : <label>{t("zipArchive")}<input required type="file" accept=".zip,application/zip" onChange={event => setArchive(event.target.files?.[0])} /><small>{t("zipHint")}</small></label>}
