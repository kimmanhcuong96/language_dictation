...
function DictationLesson({manifest,lessonSlug,lessonPath,lessonStates,onLessonState,locale,onHome}:{manifest?:LessonManifest;lessonSlug?:string;lessonPath?:{level:string;category:string;slug:string};lessonStates:LessonStateMap;onLessonState:(lessonId:string,patch:Partial<Omit<LessonState,"lessonId">>)=>void;locale:UiLocale;onHome:()=>void}){
  ...
  return <div className="dictation-shell">
    <main className="lesson-practice-page">
      ...
      {mode==="transcript"?<section className="lesson-practice-card full-transcript-view">{playableAudioSource&&!audioError?<audio key={audioAttempt} ref={transcriptAudioRef} aria-label={lessonT(locale,"fullLessonAudio")} controls preload="metadata" src={playableAudioSource} onError={handleAudioError} onLoadedMetadata={()=>setTranscriptTimeMs(0)} onTimeUpdate={event=>setTranscriptTimeMs(event.currentTarget.currentTime*1000)} onSeeked={event=>setTranscriptTimeMs(event.currentTarget.currentTime*1000)}/>:audioPlaceholder}<ol>{lesson.sentences.map((item,itemIndex)=>{const isCurrent=itemIndex===activeTranscriptIndex;return <li className={isCurrent?"current":""} aria-current={isCurrent?"step":undefined} key={item.id}><button type="button" className="transcript-sentence" onClick={()=>playTranscriptSentence(item.start_ms)}><span>{itemIndex+1}</span><p>{item.transcript}</p></button></li>})}</ol></section>:<section className="lesson-practice-card">
        ...
        <div className="practice-audio">{playableAudioSource&&!audioError?<AudioSegmentPlayer key={audioAttempt} ref={playerRef} locale={locale} src={playableAudioSource} startMs={sentence.start_ms} endMs={sentence.end_ms} playbackRate={speed} repeat={preferences.autoReplay} repeatDelayMs={preferences.replayDelaySeconds*1000} onError={handleAudioError}/>:audioPlaceholder}</div>{preferences.shortcutTips&&<div className="shortcut-key-tips"><span><kbd>{shortcutLabel(preferences.playPauseKey)}</kbd>{lessonT(locale,"playPauseShortcut")}</span><span><kbd>{shortcutLabel(preferences.replayKey)}</kbd>{lessonT(locale,"replayShortcut")}</span></div>}
