...
function DictationLesson({manifest,lessonSlug,lessonPath,lessonStates,onLessonState,locale,onHome}:{manifest?:LessonManifest;lessonSlug?:string;lessonPath?:{level:string;category:string;slug:string};lessonStates:LessonStateMap;onLessonState:(lessonId:string,patch:Partial<Omit<LessonState,"lessonId">>)=>void;locale:UiLocale;onHome:()=>void}){
  ...
  useEffect(()=>{activeStudyTimerRef.current.reset(document.visibilityState==="visible"&&document.hasFocus());setTyped("");setSubmitted(false);setRevealed(false);setAttempts(0);setProgressError(null);},[index,lesson?.id]);
  useEffect(()=>{const pause=()=>activeStudyTimerRef.current.pause();const resume=()=>{if(document.visibilityState==="visible"&&document.hasFocus())activeStudyTimerRef.current.resume();};const visibility=()=>document.visibilityState==="visible"?resume():pause();window.addEventListener("focus",resume);window.addEventListener("blur",pause);document.addEventListener("visibilitychange",visibility);resume();return()=>{pause();window.removeEventListener("focus",resume);window.removeEventListener("blur",pause);document.removeEventListener("visibilitychange",visibility);};},[]);
  ...
  const retryAnswer=async()=>{
    ...
    cancelSpeechRecognition();setProgressError(null);
    ...
    catch{setCompleted(previous);localStorage.setItem(guestKey(lesson.id),JSON.stringify([...previous]));onLessonState(lesson.id,{isCompleted:previous.size>=lesson.sentences.length});setProgressError("reset");}
  ...
  const playTranscriptSentence=(item:ListeningSentence,itemIndex:number)=>{setIndex(itemIndex);setTranscriptTimeMs(item.start_ms);if(lesson.templateType==="media"){youtubePlayerRef.current?.playSegment(item.start_ms,item.end_ms);return;}const audio=transcriptAudioRef.current;if(audio)void seekAndPlayTranscript(audio,item.start_ms).catch(()=>undefined);};
  ...
  return <div className="dictation-shell">
    <main className="lesson-practice-page">
      ...
      {mode==="transcript"?<section className="lesson-practice-card full-transcript-view">{lesson.templateType==="audio"&&(playableAudioSource&&!audioError?<audio key={audioAttempt} ref={transcriptAudioRef} aria-label={lessonT(locale,"fullLessonAudio")} controls preload="metadata" src={playableAudioSource} onError={handleAudioError} onLoadedMetadata={()=>setTranscriptTimeMs(0)} onTimeUpdate={event=>setTranscriptTimeMs(event.currentTarget.currentTime*1000)} onSeeked={event=>setTranscriptTimeMs(event.currentTarget.currentTime*1000)}/>:audioPlaceholder)}<ol>{lesson.sentences.map((item,itemIndex)=>{const isCurrent=itemIndex===activeTranscriptIndex;return <li className={`${isCurrent?"current ":""}${playedSentenceIds.has(item.id)?"played":""}`.trim()} aria-current={isCurrent?"step":undefined} data-playback-completed={playedSentenceIds.has(item.id)||undefined} key={item.id}><button type="button" className="transcript-sentence" onClick={()=>playTranscriptSentence(item,itemIndex)}><span>{itemIndex+1}</span><p>{item.transcript}</p></button></li>})}</ol></section>:<section className="lesson-practice-card">
        ...
        <div className={`dictation-workspace${accepted?" answer-visible":""}`}>
          <div className="dictation-entry">
            ...
            {progressError&&<p className="progress-reset-error" role={progressError==="reset"?"alert":"status"}>{progressError==="save"?progressSavePending[locale]:lessonT(locale,"retryAnswerError")}</p>}
      ...
      <SentenceComments sentenceId={sentence.id} sentenceNumber={index+1} locale={locale}/>
