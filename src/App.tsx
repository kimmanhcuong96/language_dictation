...
function LeaderboardLauncher({ locale, nav = false }: { locale: UiLocale; nav?: boolean }) {
  ...
  return <><a className={nav ? "nav-item platform-link platform-link-talk" : "platform-link platform-link-talk"} href="https://me2talk.com/" target="_blank" rel="noreferrer">Me2talk</a><a className={nav ? "nav-item platform-link platform-link-write" : "platform-link platform-link-write"} href="https://write.me2listen.com/" target="_blank" rel="noreferrer">Me2write</a>{nav ? <button ref={launcherRef} type="button" className="nav-item leaderboard-nav-item" onClick={() => setOpen(true)}><Trophy size={19} />{t("leaderboard")}</button> : <button ref={launcherRef} type="button" className="leaderboard-button" onClick={() => setOpen(true)} aria-label={t("leaderboard")}><Trophy size={17} /><span>{t("leaderboard")}</span></button>}{open && <LeaderboardModal locale={locale} onClose={close} />}</>;
}
