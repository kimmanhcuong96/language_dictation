...
function LeaderboardLauncher({ locale, nav = false }: { locale: UiLocale; nav?: boolean }) {
  const t = getT(locale);
  const [open, setOpen] = useState(false);
  return <>{nav ? <button className="nav-item leaderboard-nav-item" onClick={() => setOpen(true)}><Trophy size={19} />{t("leaderboard")}</button> : <button className="leaderboard-button" onClick={() => setOpen(true)} aria-label={t("leaderboard")}><Trophy size={17} /><span>{t("leaderboard")}</span></button>}<a className={nav ? "nav-item platform-link platform-link-write" : "platform-link platform-link-write"} href="https://write-checker.pages.dev/" target="_blank" rel="noreferrer">Me2write</a><a className={nav ? "nav-item platform-link platform-link-talk" : "platform-link platform-link-talk"} href="https://me2talk.com/" target="_blank" rel="noreferrer">Me2talk</a>{open && <LeaderboardModal locale={locale} onClose={() => setOpen(false)} />}</>;
}
